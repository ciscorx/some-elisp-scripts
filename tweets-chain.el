  ...
  (cond ((string-empty-p string)
	...
	((string= (substring string chars chars) " ")
	 (cons (substring string 0 chars)
               (split-string-on-word-boundaries-every (substring string chars)
                                   chars)))
	(t (let ((lastspace_pos chars))
		 ...
		 (cons (substring string 0  lastspace_pos)
		       (split-string-on-word-boundaries-every (substring string lastspace_pos)
							     chars))
					; else
		 (cons (substring string 0  chars)
		       (split-string-on-word-boundaries-every (substring string chars)
...
(defun ciscorx/split-string-on-word-boundaries-every_w_trailing_idx (string chars &optional recurse_level recurse_max)
  "Split STRING into substrings of length CHARS characters, while trying not to split up any words.
But, if it reaches a line that cannot be split up, without breaking up a word, it just accepts the line of CHARS characters as a whole.
This returns a list of strings.  Dont pass the RECURSE_LEVEL nor RECURSE_MAX parameters as those are only used internally and will cause a malfunction if supplied.  Required: split-string-on-word-boundaries-every "
  (if (not (and recurse_level recurse_max))
      (progn
	(setq recurse_max (length (split-string-on-word-boundaries-every string (- chars 6))))
	(if (> recurse_max 9)
	    (progn
	      (setq recurse_max (length (split-string-on-word-boundaries-every string (- chars 8))))
	      (split-string-on-word-boundaries-every_w_trailing_idx string (- chars 8) 1 recurse_max)
	      )
					; else
	  (split-string-on-word-boundaries-every_w_trailing_idx string (- chars 6) 1 recurse_max) 
	  )
	)
					; else
    ...
    (cond ((string-empty-p string)
	  ...
	  ((string= (substring string chars chars) " ")
	   (cons (concat (substring string 0 chars) " [" (number-to-string recurse_level) "/" (number-to-string recurse_max) "]")
		 (split-string-on-word-boundaries-every_w_trailing_idx (substring string chars)
	  ...
	  (t (let ((lastspace_pos chars))
		 ...
		 (setq lastspace_pos (1- lastspace_pos))
		   ...
		   (cons (concat (substring string 0  lastspace_pos) " [" (number-to-string recurse_level) "/" (number-to-string recurse_max) "]")
			 (split-string-on-word-boundaries-every_w_trailing_idx (substring string lastspace_pos) chars (1+ recurse_level) recurse_max
										      ))
					; else
		 (cons (concat (substring string 0  chars) " [" (number-to-string recurse_level) "/" (number-to-string recurse_max) "]")
		       (split-string-on-word-boundaries-every_w_trailing_idx (substring string chars) chars (1+ recurse_level) recurse_max
										    ))
		 )
	       ))
	  )
